...
let handler  = async (m, { conn }) => {
  conn.sendButtonLoc(m.chat, await (await fetch(fla + 'tantangan')).buffer(), `${pickRandom(global.dare)}`, '_*© 𝚏𝚊𝚑𝚛𝚒𝚕*_', 'SUDAH?', `bener`, m)
}
handler.help = ['tantangan']
handler.tags = ['game']
handler.command = /^(tantangan)$/i
handler.owner = false
handler.mods = false
