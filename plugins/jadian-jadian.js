let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    if (typeof global.db.data.user[user] == "undefined") return m.reply("*Orang yang anda tag tidak terdaftar di bot.*")
    
    if(global.db.data.user[m.sender].pasangan != "" && global.db.data.user[global.db.data.user[m.sender].pasangan].pasangan == m.sender && global.db.data.user[m.sender].pasangan != user){
      var denda = Math.ceil(global.db.data.user[m.sender].money/100*20)
      global.db.data.user[m.sender].money -= denda
      conn.reply(m.chat,`*Kamu sudah berpacaran dengan @${global.db.data.user[m.sender].pasangan.split('@')[0]}*\n\nSilahkan putus dulu (.putus) untuk menembak @${user.split('@')[0]}\n\nBtw yang setia dikit bro !\nDenda : Rp. ${format(denda)} (20% saldo)`,m,{contextInfo: {
        mentionedJid: [user,global.db.data.user[m.sender].pasangan]
      }})
    }else if(global.db.data.user[user].pasangan != ""){
      var pacar = global.db.data.user[user].pasangan
      if (global.db.data.user[pacar].pasangan == user){
        var denda = Math.ceil(global.db.data.user[m.sender].money/100*20)
        global.db.data.user[m.sender].money -= denda
        if (m.sender == pacar && global.db.data.user[m.sender].pasangan == user) return conn.reply(m.chat,`*Anda sudah berpacaran dengan @${beb.split('@')[0]}*\n\n*Yang setia jadi orang bosss*\n\n*Denda : Rp. ${format(denda)} (20% saldo)*`,m,{contextInfo: {
          mentionedJid: [beb]
        }})
        ...
        global.db.data.user[m.sender].pasangan = user
    ...
    }else if (global.db.data.user[user].pasangan == m.sender){
      global.db.data.user[m.sender].pasangan = user
